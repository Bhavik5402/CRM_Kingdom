...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { pageid: 0, text: "Team Management", icon: <PeopleIcon />,
        submenu: [
            { text: "Team", path: "/users", icon: <PeopleIcon /> },
            { text: "Add Team Member", path: "/users/add-user", icon: <PersonAddIcon /> },
          ],
          handleOpenFunc : handleUserClick,
          pageState : openUserMenu
     },
    { pageid: 1, text: "Stage", icon: <LayersIcon /> ,
        submenu: [
            { text: "Stage List", path: "/stage", icon: <LayersIcon /> },
            { text: "Add Stage", path: "/stage/add", icon: <AddToPhotosIcon /> },
          ],
    ...
    },
    {
      ...
      submenu: [
        { text: "Lead List", path: "/leads", icon: <LayersIcon /> },
        { text: "Create Lead", path: "/leads/add", icon: <AddCircleIcon /> },
          ...
          .map((item) => (
            <React.Fragment key={item.text}>
              ...
              >
                ...
                {item.submenu ? (item.pageState ? <ExpandLess /> : <ExpandMore />) : null}
              </ListItem>
              {item.submenu && (
                <Collapse in={item.pageState} timeout="auto" unmountOnExit>
                  <List component="div" disablePadding className="sidebar-menu-list-custom">
                    {item.submenu.map((submenuItem) => (
                      <ListItem
                        ...
                        component={Link}
                        to={submenuItem.path}
                        className={location.pathname === submenuItem.path ? "active submenu-item" : " submenu-item"}
                      >
                        <div className="menu-icon">{submenuItem.icon}</div>
                        <ListItemText className="sidebarmenutext" primary={submenuItem.text} />
                      </ListItem>
                    ))}
                  </List>
                </Collapse>
